...
export async function saveFile(file: Express.Multer.File, options: SaveFileOptions): Promise<string> {
    ...
    try {
        // Resolve absolute destination path (sirf saving ke liye)
        ...
        // Relative path (DB me store karne layak, clean & cross-platform)
        const relativePath = path.join(options.destination, filename).split(path.sep).join('/');
        debug("Relative path:", relativePath);

        // ✅ APP_URL ke sath full URL bana ke return karo
        const baseUrl = (process.env.APP_URL || '').replace(/\/$/, '');
        const cleanRelativePath = relativePath.replace(/^\/+/, '');
        const fullUrl = `${baseUrl}/${cleanRelativePath}`;
        debug("Full URL (returned):", fullUrl);

        debug("----- saveFile END -----");
        return fullUrl;   // 👈 ab pura URL directly return hoga
