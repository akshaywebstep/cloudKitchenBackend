...
const ingredientSchema = z.object({
    ...
    quantity: z.coerce.number().positive("Ingredient quantity must be greater than 0"),
});
...
export const createMenuItemSchema = z.object({
    ...
    // ✅ ab payload se aayenge
    kitchenId: z.coerce.number().positive("kitchenId is required"),
    branchId: z.coerce.number().positive("branchId is required"),
...
export const validateCreateMenuItem = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!result.success) {
        return res.status(400).json({ status: false, message: 'Validation failed', errors: result.error.flatten().fieldErrors });
    }
...
export const validateMenuItemId = (req: Request, res: Response, next: NextFunction) => {
    const result = menuItemIdSchema.safeParse(req.params);
    if (!result.success) {
        return res.status(400).json({ status: false, message: 'Invalid menu item id' });
    }
    next();
};

export const validateMenuItemStatus = (req: Request, res: Response, next: NextFunction) => {
