...
// ➕ CREATE KITCHEN ROLE
export const createKitchenRole = async (req: Request, res: Response) => {
    debugHelper.debug('=== CREATE KITCHEN ROLE START ===');
    ...
    try {
        ...
        const result = await StaffRoleService.createKitchenRole(userId, name);
        const cleanData = stringHelper.convertBigInt(result);

        return res.status(201).json({
            status: true,
            message: 'Kitchen role created successfully',
            data: cleanData
        });
    } catch (error: any) {
        ...
        debugHelper.debug('=== CREATE KITCHEN ROLE END ===');
    }
};
