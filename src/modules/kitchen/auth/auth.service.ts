  ...
  try {
    ...
    const response = await userRepo.findFirst({
      ...
      select: {
        ...
        parent: {
          // 👈 ADD — parent relation fetch karo
          select: {
            id: true,
            userType: true,
            status: true,
    ...
    return {
      ...
      data: { kitchen: userWithoutPassword, token },
    };
  } catch (error: any) {
...
export const getUserByUsername = async (
  ...
  try {
    ...
    debugHelper.debugError(
      `[Auth Service] getUserByUsername failed: ${error.message}`,
    );
...
export const getUserByResetToken = async (token: string) => {
  try {
    ...
    debugHelper.debugError(
      `[Auth Service] getUserByResetToken failed: ${error.message}`,
    );
...
export const createKitchen = async (data: {
  ...
  debugHelper.debug(
    `[Kitchen Service] Checking if user exists: ${email} or ${phone}`,
  );
  ...
  if (existingUsers.data && existingUsers.data.length > 0) {
    ...
    debugHelper.debugError(
      `[Kitchen Service] Creation failed: ${errorMessage}`,
    );
  ...
  if (!createResponse.status) {
    return {
      status: false,
      message: createResponse.message || "Failed to create kitchen",
    };
  }

  debugHelper.debug(
    `[Kitchen Service] Kitchen created: ID ${createResponse.data.id}`,
  );
