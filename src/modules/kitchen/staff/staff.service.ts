...
export const createStaff = async (data: {
    ...
    try {
        // ✅ 1. Validate Required Profile Picture
        if (!profilePicture) {
        ...
        // ✅ 4. BranchIds compulsory + must belong to this kitchen
        if (!branchIds || branchIds.length === 0) {
            return { status: false, message: "At least one branch must be selected" };
        }
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            // Branch access ab guaranteed create hoga kyunki branchIds compulsory hai
