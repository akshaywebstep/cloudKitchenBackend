...
const preprocessBranchIds = z.preprocess((val) => {
  ...
  return val;
}, z.array(z.coerce.number().positive())); // 👈 .optional() hata diya
...
export const createStaffSchema = z.object({
 ...
 branchIds: preprocessBranchIds.pipe(
  z.array(z.number().positive()).min(1, "At least one branch must be selected")
),
});
